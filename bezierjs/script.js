	...
	if (evt.key !== undefined) {
		...
		switch(evt.key){
			...
			case 'l':
				...
				mybezier.curves.length = 0;
			        
					let pts = e.points;
					mybezier.curves.push(new Bezier(pts[0][0],pts[0][1],pts[1][0],pts[1][1],pts[2][0],pts[2][1],pts[3][0],pts[3][1]));
					mybezier.curves[mybezier.curves.length-1].showbbx = e.showbbx;
					mybezier.curves[mybezier.curves.length-1].mouse = e.mouse;
					mybezier.curves[mybezier.curves.length-1].outlinemin = e.outlinemin;
					mybezier.curves[mybezier.curves.length-1].outlinemax = e.outlinemax;
					mybezier.curves[mybezier.curves.length-1].color = e.color;	
			...
			case 'r':
				...
				mybezier.curves.length = 0;
					...
					pts.push([getRandomX(),getRandomY()])
					mybezier.curves.push(new Bezier(pts[0][0],pts[0][1],pts[1][0],pts[1][1],pts[2][0],pts[2][1],pts[3][0],pts[3][1]));
					mybezier.curves[mybezier.curves.length-1].showbbx = false;
					mybezier.curves[mybezier.curves.length-1].mouse = false;
					mybezier.curves[mybezier.curves.length-1].outlinemin = getRandomThick();
					mybezier.curves[mybezier.curves.length-1].outlinemax = getRandomThick();
					mybezier.curves[mybezier.curves.length-1].color = getRandomColor();	
				}
...
function addBezier(canvas,x1,y1,x2,y2,x3,y3,x4,y4,color){
  ...
  mybezier.curves.push(new Bezier(x1,y1,x2,y2,x3,y3,x4,y4));
  mybezier.curves[mybezier.curves.length-1].showbbx = false;
  mybezier.curves[mybezier.curves.length-1].mouse = false;
  mybezier.curves[mybezier.curves.length-1].outlinemin = 1;
  mybezier.curves[mybezier.curves.length-1].outlinemax = 25;
  mybezier.curves[mybezier.curves.length-1].color = color;
